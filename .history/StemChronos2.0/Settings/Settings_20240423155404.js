...
function SetDefaultSelect() {
  ...
  if (localStorage.getItem("TimeMode") != null) {
    ...
    document.getElementById("TimeMode").value =
      localStorage.getItem("TimeMode"); //set the time mode to it
  }
}
...
function AddEventListeners() {
  ...
  document.getElementById("ScheduleBTN").addEventListener("click", () => {
    BackToSchedule();
  }); //Go back to schedule BTN

  for (var i = 0; i < PeriodID.length; i++) {
    //Add event listener for all schedule name inputs
    document
      .getElementById(PeriodID[i])
      .addEventListener("change", (source) => {
  ...
  for (let i = 0; i < PeriodInputID.length; i++) {
    //Add eventlisteners to all schedule input toggle arrows
    const element = PeriodInputID[i]; //id for element

    document.getElementById(element).addEventListener("click", (source) => {
      //add the listener and pass the object
...
function SetDateAndTime() {
  ...
  document.getElementById("CurrentDate").innerHTML =
    AllMonths[month] + " " + day + ", " + year; //set the day month, year display

  if (TimeMode === null || TimeMode === "12Hour") {
    if (hour > 12) hour -= 12;
    else if (hour === 0) hour = 12;
  }
  minutes = (minutes < 10 ? "0" : "") + minutes; // Add leading zero if minutes < 10
