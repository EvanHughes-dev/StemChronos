...
async function SetSchedule(Schedule) {
  ...
  var LetterDay = currentData.LetterDay;
  if (LetterDay == "X") {
    return;
  }
  ...
  try {
    ...
    var PeriodNames = currentData.PeriodNames.split("/");
    var PeriodTimes = currentData.PeriodTimes.split("/");
    //Period names and times are stored each as a single string
    //each break is a /
    ...
    for (var i = 0; i < PeriodNames.length; i++) {
      ...
      var Times = PeriodTimes.split("-");
      var startHour = Times[0].toString().split(":")[0];
      var endHour = Times[1].toString().split(":")[0];
      var endMinute = Times[1].toString().split(":")[1];
      var startMinute = Times[0].toString().split(":")[1];
      ...
      if (hour >= startHour && hour <= endHour && !foundCurrentPeriod) {
        ...
        } else if (hour == endHour && minutes < endMinute) {
          //if the hour is just equal to the end hour
    ...
    if (hour < firstHour || (hour == firstHour && minutes < firstMinute)) {
      var minutesUnitilSchoolStart =
        firstHour * 60 + parseInt(firstMinute) - (hour * 60 + minutes);
      ...
      if (minutesUnitilSchoolStart / 60 < 1) {
        ...
        var hoursLeft =
          minutesUnitilSchoolStart - (minutesUnitilSchoolStart % 60);
        hoursLeft /= 60;
        if (hoursLeft > 1) {
          Remianing.innerHTML +=
            hoursLeft +
            " hours " +
            (minutesUnitilSchoolStart % 60) +
            " minutes";
        } else {
          Remianing.innerHTML +=
            hoursLeft + " hour " + (minutesUnitilSchoolStart % 60) + " minutes";
        }
      }
...
function AddNewElement(table, PeriodName, PeriodTime, currentPeriod) {
  if (
    ...
    if (
      window.localStorage.getItem("Period" + PeriodName.substring(0, 1)) != null
    ) {
      PeriodName = window.localStorage.getItem(
        "Period" + PeriodName.substring(0, 1)
      );
    }
  }
